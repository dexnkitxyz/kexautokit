...
class CollapsibleFrame(tk.Frame):
    """Frame ที่พับ/ขยายได้พร้อมลูกศร ▼/▶"""
    
    def __init__(self, parent, title="", tooltip=None, **kwargs):
...
class AutoReceiveApp:
    def __init__(self, root):
        self.root = root
        root.title("Auto รับสินค้าจากบาร์โค้ด PDF")
    ...
    def build_ui(self):
        ...
        ttk.Separator(toolbar, orient="vertical").pack(side="left", fill="y", padx=6, pady=4)
        ttk.Button(toolbar, text="📋 รายการบาร์โค้ด", command=self.show_codes_window).pack(
            side="left", padx=2, pady=4
        )
        ...
        frm_file = CollapsibleFrame(self.root, title="1) ไฟล์และโปรแกรมเป้าหมาย")
        ...
        ttk.Label(frm_file.inner_frame, text="ชื่อหน้าต่างโปรแกรมเป้าหมาย:").grid(row=3, column=0, sticky="w")
        self.window_combo = ttk.Combobox(frm_file.inner_frame, textvariable=self.window_title_var, width=52)
        self.window_combo.grid(row=3, column=1, sticky="we")
        ttk.Button(frm_file.inner_frame, text="รีเฟรชรายชื่อหน้าต่าง", command=self.refresh_windows).grid(row=3, column=2)

        frm_file.inner_frame.columnconfigure(1, weight=1)

        frm_val = CollapsibleFrame(self.root, title="2) ค่าเริ่มต้นและจังหวะเวลา")
        ...
        seq_tree_frame.grid(row=1, column=0, columnspan=5, sticky="we", padx=4, pady=4)
        self.seq_tree = ttk.Treeview(
            seq_tree_frame, columns=("action", "detail"), show="tree headings", height=6, selectmode="browse"
        )
        ...
        frm_run = CollapsibleFrame(self.root, title="4) ตัวเลือกก่อนเริ่มทำงาน")
        ...
        frm_ocr = CollapsibleFrame(
            self.root, title="5) ตรวจจับข้อความบนจอ (OCR)",
            tooltip="ใช้กับสเต็ป 'รอข้อความ (OCR)' ในข้อ 3",
        )
        frm_ocr.pack(fill="both", expand=False, **pad)
        ...
        frm_verify = CollapsibleFrame(
            self.root, title="6) ตรวจสอบรายการที่ตกหลุด",
            tooltip="ใช้หลังกรอกข้อมูลครบทุกรายการแล้ว — เทียบรายการที่ดึงจาก PDF กับสิ่งที่ปรากฏจริงในตารางของโปรแกรมเป้าหมาย",
        )
        frm_verify.pack(fill="both", expand=False, **pad)
        ...
        frm_log = CollapsibleFrame(self.root, title="Log")
    ...
    # ---------- เมนูบาร์ + รายการบาร์โค้ดที่ดึงมา ----------
    def setup_menu(self):
        menubar = tk.Menu(self.root)
        view_menu = tk.Menu(menubar, tearoff=0)
        view_menu.add_command(label="ดูรายการบาร์โค้ดที่ดึงมา", command=self.show_codes_window)
        shortcut_menu = tk.Menu(view_menu, tearoff=0)
        shortcut_menu.add_command(label="ตั้งค่าปุ่มลัด...", command=self.open_shortcuts_window)
        view_menu.add_cascade(label="ปุ่มลัด", menu=shortcut_menu)
        menubar.add_cascade(label="เมนู", menu=view_menu)
        self._real_root.config(menu=menubar)
    ...
    def apply_shortcuts(self):
        ...
        for action, callback in callbacks.items():
            ...
            if sequence:
                self._real_root.bind_all(sequence, lambda _event, cb=callback: cb())
    ...
    def _capture_shortcut(self, parent, variable):
        ...
        dialog.bind("<KeyPress>", lambda event: (variable.set(self._format_shortcut_event(event)), dialog.destroy()))
        dialog.focus_force()
    ...
    def show_missing_report(self, missing, total):
        ...
        def copy_list():
            try:
                ...
                pyperclip.copy("\n".join(missing_codes))
                self.extracted_codes = missing_codes
                self.save_codes_cache()
                self.update_codes_count_label()
                self.log(f"ตั้งรายการที่ตกหลุดเป็นรายการหลักแล้ว ({len(missing_codes)} รายการ)")
                messagebox.showinfo(
                    "คัดลอกแล้ว",
                    f"คัดลอกรายการที่ตกหลุด {len(missing_codes)} รายการไปยังคลิปบอร์ดแล้ว\n"
                    "รายการนี้ถูกตั้งเป็นรายการหลักสำหรับการกดเริ่มครั้งถัดไปแล้ว",
                )
            except Exception as e:
